...
public class Title : MonoBehaviour {
	...
	void Stamp()
	{
		if (Time.time > startTime + 2.0f) {
			prototypeLabel.transform.position = Vector3.Lerp (prototypeLabel.transform.position, new Vector3(prototypeLabel.transform.position.x, prototypeLabel.transform.position.y, 0), 0.2f);
			prototypeLabel.transform.localScale = Vector3.Lerp (prototypeLabel.transform.localScale, new Vector3(3,3,1), 0.2f);

			if(SimpleGesture.instance.GetGesture () == InputState.Tap)
			{
